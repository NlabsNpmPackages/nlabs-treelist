import { Component, Input, Output, EventEmitter, OnInit, OnChanges, SimpleChanges, HostBinding, TemplateRef, ContentChild } from '@angular/core';
...
export class NlabsTreeListComponent implements OnInit, OnChanges {
  ...
  @Input() imageField: string = 'imageUrl'; // Görsel URL alanı
  @Input() childrenField: string = 'children';
  @Input() showImage: boolean = false; // Görsel gösterilsin mi?
  @Input() showActions: boolean = false; // CRUD butonları gösterilsin mi?

  // Custom template support
  @ContentChild('nodeContent') nodeContentTemplate?: TemplateRef<any>;
  ...
  getImageUrl(node: any): string | undefined {
    return node?.[this.imageField];
  }
  ...
  // CRUD action events
  @Output() nodeAdd = new EventEmitter<any>(); // Parent node'a child ekle
  @Output() nodeEdit = new EventEmitter<any>(); // Node'u düzenle
  @Output() nodeDelete = new EventEmitter<any>(); // Node'u sil
  ...
  private async toggleNode(node: any) {
    if (node.expanded) {
      node.expanded = false;
      this.nodeCollapsed.emit(node);
    } else {
  ...
  // CRUD action methods
  onAddChild(node: any, event: Event) {
    event.stopPropagation(); // Parent tıklamasını engelle
    this.nodeAdd.emit(node);
  }

  onEditNode(node: any, event: Event) {
    event.stopPropagation();
    this.nodeEdit.emit(node);
  }

  onDeleteNode(node: any, event: Event) {
    event.stopPropagation();
